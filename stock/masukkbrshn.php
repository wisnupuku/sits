...
<?php 
    ...
    if(isset($_POST['update'])){
        ...
        $lihatstock = mysqli_query($conn,"select * from sits_invkbrshn where idx='$idx'"); //lihat stock barang itu saat ini
        $stocknya = mysqli_fetch_array($lihatstock); //ambil datanya
        $stockskrg = $stocknya['stock'];//jumlah stocknya skrg

        $lihatdataskrg = mysqli_query($conn,"select * from masukkebersihan where id='$id'"); //lihat qty saat ini
        ...
        if($jumlah >= $qtyskrg){
            ...
            $queryx = mysqli_query($conn,"update sits_invkbrshn set stock='$tambahistock' where idx='$idx'");
            $updatedata1 = mysqli_query($conn,"update masukkebersihan set nama_barang='$nama_barang', tglmasuk='$tglmasuk',jumlah='$jumlah' where id='$id'");
            ...
            if ($updatedata1 && $queryx){
                ...
                <meta http-equiv='refresh' content='1; url= masukkbrshn.php'/>  ";
                } else { echo "<div class='alert alert-warning'>
                    <strong>Failed!</strong> Redirecting you back in 3 seconds.
                </div>
                <meta http-equiv='refresh' content='3; url= masukkbrshn.php'/> ";
                };
            ...
            $query1 = mysqli_query($conn,"update sits_invkbrshn set stock='$kurangistock' where idx='$idx'");

            $updatedata = mysqli_query($conn,"update masukkebersihan set tglmasuk='$tglmasuk', jumlah='$jumlah' where id='$id'");
    ...
    if(isset($_POST['hapus'])){
        ...
        $queryx = mysqli_query($conn,"update sits_invkbrshn set stock='$adjuststock' where idx='$idx'");
        $del = mysqli_query($conn,"delete from masukkebersihan where id='$id'");
                                ...
                                <div class="market-status-table mt-4">
                                    <div class="table-responsive">
                                        <table id="dataTable3" class="table table-hover">
                                            ...
                                            <tbody>
                                                <?php 
											$brg=mysqli_query($conn,"SELECT * from  masukkebersihan sb, sits_invkbrshn st where st.idx=sb.idx order by sb.id DESC");
											$no=1;
                                    ...
                                    <a href="export_masukkbrshn.php" target="_blank" class="btn btn-info">Export Data</a>
                                </div>
                            </div>
                        </div>
                    </div>
